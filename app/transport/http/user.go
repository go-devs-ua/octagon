...
func (uh UserHandler) CreateUser() http.Handler {
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		if err = json.NewDecoder(req.Body).Decode(&user); err != nil {
			WriteJSONResponse(rw, http.StatusBadRequest, Response{MsgBadRequest}, uh.logger)
		...
		if err = user.Validate(); err != nil {
			WriteJSONResponse(rw, http.StatusBadRequest, Response{"Validation error: " + err.Error()}, uh.logger)
		...
		if err = uh.usecase.Signup(user); err != nil {
			...
			if err, ok := errors.Unwrap(err).(*pq.Error); ok && err.Code.Name() == "unique_violation" {
				WriteJSONResponse(rw, http.StatusConflict, Response{MsgEmailConflict}, uh.logger)
				return
			}

			WriteJSONResponse(rw, http.StatusInternalServerError, Response{MsgInternalSeverErr}, uh.logger)
			return
		}

		WriteJSONResponse(rw, http.StatusCreated, Response{MsgUserCreated}, uh.logger)
